...
def mostrar_modulo_ventas(ultimo_calculo=None):
    ...
    if not df_inv.empty:
        ...
        if "Estado" in df_inv.columns:
            df_activos = df_inv[
                df_inv["Estado"].astype(str).str.strip().str.capitalize()
                == "Activo"
            ]
    ...
    if not df_ventas.empty:
        ...
        # Asegurar que total_ingresos sea un número flotante válido de forma segura
                        ...
                        for parte in partes:
                            if "(" in parte and ")" in parte:
                                ...
                                try:
                                    ...
                                    if res.data:
                                        actuales = float(
                                            res.data[0]["Gramos_Actuales"]
                                        )
                                        supabase.table("inventario").update(
                                            {
                                                "Gramos_Actuales": actuales
                                                + g_dev,
                                                "Estado": "Activo",
                                            }
                                        ).eq("ID_Rollo", id_r).execute()
